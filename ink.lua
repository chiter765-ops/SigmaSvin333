...
*    Ink Game ULTIMATE v8.6.1 — SAFERCORE — Startup-Safe Repair*

*    Target place: 99567941238278*

*    Design goals:*

*      * No guessed server-remote arguments.*

*      * No anti-cheat bypass.*

*      * Drives visible client mechanics through GUI/input only.*

*      * Incremental GUI index (no repeated GetDescendants() hot-loop).*

*      * Current-game detection includes Jump Rope / Last Dinner / Final Fight.*

*      * QTE parser supports button prompts, single-key prompts, number sequences,*

*        timing bars, and reused GUI instances.*

*      * Pentathlon solver uses the actual five mechanics instead of a generic*

*        "click Complete" state machine.*

*      * Reinjection-safe runtime and bounded work per frame.*
...
if not LocalPlayer then
    ...
    warn("[Ink v8.6.1] LocalPlayer is unavailable; bootstrap aborted safely.")
    return
end
...
if not PlayerGui then
    ...
    if rawget(_G, RUNTIME_KEY) == runtime then _G[RUNTIME_KEY] = nil end
    warn("[Ink v8.6.1] PlayerGui was unavailable after 10 seconds; bootstrap aborted safely.")
    return
end
...
if game.PlaceId ~= TARGET_PLACE_ID then
    ...
    warn(("[Ink v8.6.1] Expected place %d, running in %d. Heuristics may not match.")
...
local function virtualInputManager()
    if vimResolved then return VIM end
    vimResolved = true
    ...
    VIM = ok and result or nil
    return VIM

end

-- Wall-clock input holds are intentional latency, not CPU work.
...
local function mouseClick(position)
    ...
    position = position or (cam and cam.ViewportSize / 2) or Vector2.new(1, 1)

    if vim then
        local downSent = false
        ...
        if downSent then
            releaseOk = pcall(function()
                vim:SendMouseButtonEvent(position.X, position.Y, 0, false, game, 0)
            end)
        end

        if ok and releaseOk then return true end

        -- If VIM accepted mouse-down, do not fall through to a second input
        -- backend. A failed release is reported as failure after one best-effort
        -- extra mouse-up, preventing duplicate clicks/held-state amplification.
        if downSent then
            pcall(function()
                vim:SendMouseButtonEvent(position.X, position.Y, 0, false, game, 0)
            end)
    ...
    local ok = pcall(function()
        ...
        vu:Button1Down(position, cam and cam.CFrame or CFrame.new())
        downSent = true
        trackedInputWait(0)
    end)

    local releaseOk = true
    if downSent and vu then
        releaseOk = pcall(function()
            vu:Button1Up(position, cam and cam.CFrame or CFrame.new())
        end)
    end

    return ok and releaseOk

end

local function keyTap(keyCode, holdTime)

    holdTime = holdTime or 0.018
    local vim = virtualInputManager()
    ...
    if downSent and not releaseOk then
        pcall(function()
            vim:SendKeyEvent(false, keyCode, false, game)
        end)
    end
...
local function keyDown(keyCode)
    ...
    return pcall(function() vim:SendKeyEvent(true, keyCode, false, game) end)
...
local function keyUp(keyCode)
    ...
    return pcall(function() vim:SendKeyEvent(false, keyCode, false, game) end)
...
local function clickGuiButton(button)
    if not button
        ...
        or button.Active == false
        or not isActuallyVisible(button) then
        return false
    end

    local point = centerOf(button)
    if not pointInteractableFor(button, point) then
        return false
    end

    -- Avoid firesignal. Direct signal re-entry can execute game UI handlers
    -- in an unexpected state; use the ordinary virtual pointer path instead.
...
local function releaseVirtualMouse()
    ...
    local p = runtime.virtualMousePosition or Vector2.new(1, 1)
    local ok = pcall(function()
    ...
    if ok then
        runtime.virtualMouseHeld = false
        runtime.virtualMousePosition = Vector2.new(1, 1)
    end
...
local function dragCircle(center, radius, loops, pointsPerLoop, shouldContinue)
    ...
    if runtime.virtualMouseHeld and not releaseVirtualMouse() then
        return false
    end

    loops = loops or 2
    pointsPerLoop = pointsPerLoop or 42
    radius = math.max(radius or 30, 12)

    local start = center + Vector2.new(radius, 0)
    local finalPosition = start
    local downSent = false
    local completed = false

    local ok = pcall(function()
        vim:SendMouseMoveEvent(start.X, start.Y, game)
        vim:SendMouseButtonEvent(start.X, start.Y, 0, true, game, 0)

        downSent = true
        runtime.virtualMouseHeld = true
        runtime.virtualMousePosition = start

        local total = math.max(8, math.floor(pointsPerLoop * loops))

        for i = 1, total do
            if not runtime.alive then return end
            if shouldContinue and not shouldContinue() then return end

            local angle = (i / pointsPerLoop) * math.pi * 2
            ...
            trackedInputWait(0.012)
        end

        completed = true
    end)

    -- finally: once mouse-down was emitted, mouse-up is attempted on every
    -- success, cancellation, unload, token change, and error path.
    local releaseOk = true
    if downSent then
        runtime.virtualMousePosition = finalPosition
        releaseOk = releaseVirtualMouse()
    end

    return ok and completed and releaseOk
...
-- v8.6.1: semantic identity is deliberately insensitive to dynamic numeric
...
local function semanticPromptText(text)
    ...
    local key = value:match("press%s+([qerfwasdzxcv])")
    if key then
        return "key:" .. key
    end
...
local function queueGuiMutation(obj, expandSubtree, rearmPrompt, semanticCheck, subtreeRearm)
    ...
    if not runtime.alive or not obj or not obj.Parent then
        return
    end
...
function F.queueNewGuiEvent(obj)
    if not runtime.alive or not obj then return end
...
function F.onGuiMutation(
    ...
    if not runtime.alive or not obj or not obj.Parent or isOwnGuiObject(obj) then return end
...
function runtime.snapshotGuiChildren(obj)
    ...
    if not ok or type(children) ~= "table" then
        if CONFIG.debug then
            warn("[Ink v8.6.1] GetChildren snapshot failed:", children)
        end
        return {}
    end
    ...
    if elapsedMs >= CONFIG.performance.slowWorkerMs then
        ...
        if CONFIG.debug then
            warn(("[Ink v8.6.1] slow GetChildren snapshot: %.2fms (%d children)")
...
function F.processIndexQueue()
    ...
    if indexHead > indexTail then
        ...
        if not guiIndexCompletionReported then
            ...
            print(("[Ink v8.6.1] GUI index complete | indexed=%d watchers=%d " ..
...
function F.pumpGuiEvents(epoch)
    if not guiIndexStarted then return 0 end
...
function F.refreshMode()
    ...
    if modeState.current ~= "Unknown"
        ...
        if CONFIG.debug then
            print("[Ink v8.6.1] Mode -> Unknown (evidence timeout)")
        end
    end
...
function F.keyFromPrompt(text)
    ...
    local t = F.normalizedPromptText(text)

    if KEY_MAP[t] then return KEY_MAP[t] end

    local key = t:match("press%s+([qerfwasdzxcv])")

    if key and KEY_MAP[key] then return KEY_MAP[key] end

    if t:find("press space", 1, true) or t:find("jump", 1, true) then
...
function F.actionAllowed(key, interval)
    ...
    if now - lastFailure < 0.025 then return false end
...
function F.tryAction(key, interval, fn)
    if not F.actionAllowed(key, interval) then
        return false
    end
...
function F.solveTimingBar(preferYellow)
    ...
    for i = 1, #objects do
        ...
        if size.X >= 8 and size.Y >= 4 and size.X <= 800 and size.Y <= 500 then
    ...
    for ti = 1, math.min(#targets, 18) do
        ...
        local tolerance = math.max(4, math.min(targetSize.X, targetSize.Y) * CONFIG.pentathlon.timingTolerance)
...
function F.solveDigitSequences(owner)
    if sequenceBusy then return false end
    ...
    for i = 1, #objects do
        ...
        if obj:IsA("TextLabel") or obj:IsA("TextButton") then
            ...
            if seq then
                ...
                local ancestorEpoch = F.ancestorVisibilityEpoch(obj)
                local state = sequenceState[obj]
                local canRun = false

                if not state
                    or state.seq ~= seq
                    or state.generation ~= generation
                    or state.ancestorEpoch ~= ancestorEpoch then
                    canRun = true

                elseif now - (state.lastSeenAt or -math.huge) > 0.90 then
                    canRun = true
                end
                ...
                if state
                    ...
                    and state.ancestorEpoch == ancestorEpoch then
                    state.lastSeenAt = now
                end

                local cooldownKey = table.concat({
                    "seq",
                    tostring(obj),
                    seq,
                    tostring(generation),
                    tostring(ancestorEpoch),
                }, ":")

                if canRun and F.actionAllowed(cooldownKey, 0.30) then
                    sequenceState[obj] = {
                        seq = seq,
                        generation = generation,
                        ancestorEpoch = ancestorEpoch,
                        lastSeenAt = now,
                    }

                    sequenceRunToken = sequenceRunToken + 1
                    local myToken = sequenceRunToken
                    local expectedGeneration = generation
                    local expectedAncestorEpoch = ancestorEpoch
                    local expectedSequence = seq

                    sequenceBusy = true

                    task.spawn(function()
                        local completedDigits = 0

                        local function stillValid()
                            if not runtime.alive or myToken ~= sequenceRunToken then
                                return false
                            end

                            if not obj or not obj.Parent or not isActuallyVisible(obj) then
                                return false
                            end

                            if (promptGeneration[obj] or 0) ~= expectedGeneration then
                                return false
                            end

                            if F.ancestorVisibilityEpoch(obj) ~= expectedAncestorEpoch then
                                return false
                            end

                            if F.extractDigitSequence(
                                textOf(obj),
                                obj,
                                sequenceOwner
                            ) ~= expectedSequence then
                                return false
                            end

                            if sequenceOwner == "gonggi" then
                                return CONFIG.enabled.autoPentathlon
                                    and modeState.current == "Pentathlon"
                                    and lastPentathlonStage == "gonggi"
                            end

                            return CONFIG.enabled.autoQTE
                                and not (
                                    CONFIG.enabled.autoPentathlon
                                    and modeState.current == "Pentathlon"
                                )
                        end

                        for j = 1, #expectedSequence do
                            if not stillValid() then break end

                            local code = DIGIT_KEYS[expectedSequence:sub(j, j)]

                            if not code or not keyTap(code) then
                                break
                            end

                            completedDigits = j

                            if j < #expectedSequence then
                                trackedInputWait(CONFIG.qte.sequenceDelay)
                            end
                        end

                        local sequenceCompleted = completedDigits == #expectedSequence

                        if sequenceCompleted then
                            -- A sequence consumes its success cooldown only when
                            -- every digit was emitted successfully.
                            F.commitAction(cooldownKey)
                        else
                            -- Cancellation and partial input are failures. Clear
                            -- matching state so an unchanged visible prompt can
                            -- retry after the short failure backoff.
                            F.failAction(cooldownKey)

                            local current = sequenceState[obj]
                            if current
                                and current.seq == expectedSequence
                                and current.generation == expectedGeneration
                                and current.ancestorEpoch == expectedAncestorEpoch then
                                sequenceState[obj] = nil
                            end
                        end

                        if myToken == sequenceRunToken then
                            sequenceBusy = false
                        end
                    end)
...
function F.solveVisibleQTEButtons()
    ...
    for i = 1, #objects do
        ...
        if obj:IsA("GuiButton") then
            ...
            local oneChar = F.normalizedPromptText(textOf(obj)):match("^[qerfwasdzxcv]$") ~= nil

            if oneChar or containsAny(descriptor, words) then
...
function F.solveGenericQTE()
    ...
    -- One successful input per pass is enough. v8.0 ran every solver even after
    -- a hit, multiplying GUI work and occasionally issuing conflicting inputs.
    if F.solveDigitSequences("qte") then return true end
    ...
    task.spawn(function()
        local callOk, dragOk = pcall(function()
            return dragCircle(
                ...
                24,
                function()
                    return runtime.alive and myToken == windingToken
                end
            )
        end)
...
function F.solvePentathlon()
    ...
    if stage == "ddakji" then
        F.clickFirstAction({"start", "ready", "throw", "ddakji"}, "ddakji-start", 0.45)
    ...
    elseif stage == "flying_stone" then
        F.clickFirstAction({"start", "ready", "throw"}, "stone-start", 0.45)
        return F.solveTimingBar(false)
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if p ~= LocalPlayer then
            ...
            if char then
                ...
                if hideSeekOnly then

                    local seeker = F.playerLooksLikeSeeker(p)

                    hl.FillColor = seeker and Color3.fromRGB(255, 75, 75) or Color3.fromRGB(70, 145, 255)
...
local parented = pcall(function() screen.Parent = CoreGui end)

if not parented then screen.Parent = PlayerGui end
...
title.Text = "Ink ULTIMATE v8.6.1 STARTUP-SAFE"
...
list.Position = UDim2.new(0, 10, 0, 68)
...
function F.toggleRow(label, field)
    ...
    trackConnection(button.MouseButton1Click:Connect(function()
        ...
        CONFIG.enabled[field] = not CONFIG.enabled[field]

        if CONFIG.enabled[field]
            and (field == "autoPentathlon"
                or field == "autoQTE"
                or field == "autoDodge"
                or field == "aimAssist"
                or field == "rebelAim"
                or field == "hideSeekESP") then
            F.ensureGuiIndexing()
        end

        if not CONFIG.enabled[field] and field == "autoPentathlon" then
            F.resetPentathlonInputs()
            F.cancelDigitSequence()
        elseif not CONFIG.enabled[field] and field == "autoQTE" then
            F.cancelDigitSequence()
        end

        if not CONFIG.enabled[field] and (field == "esp" or field == "hideSeekESP") then
            F.clearESP()
        end
...
F.toggleRow("🏆 Auto Pentathlon (mechanic-aware)", "autoPentathlon")

F.toggleRow("⌨ Auto QTE / timing", "autoQTE")
...
F.toggleRow("🔍 Hide & Seek ESP", "hideSeekESP")

F.toggleRow("👁 Player ESP", "esp")
...
function F.profiledCall(name, fn)
    ...
    if computeMs >= CONFIG.performance.slowWorkerMs then
        ...
        if CONFIG.debug then
            warn(("[Ink v8.6.1] slow CPU worker %s: %.2fms (wall %.2fms)")
                :format(name, computeMs, wallMs))
        end
    end

    if not ok and CONFIG.debug then
        warn("[Ink v8.6.1] worker error [" .. tostring(name) .. "]:", result)
    end
...
-- Cooperative scheduler: expensive work is not attached to every Heartbeat.
task.spawn(function()
    while runtime.alive do
        schedulerEpoch = schedulerEpoch + 1

        local now = os.clock()
        local active = F.anyFeatureEnabled()

        -- Covers UI toggles and programmatic CONFIG changes alike. Features that
        -- depend on GUI-derived mode evidence cannot run with an idle GUI index.
        if active and F.needsGuiIndexing() and not guiIndexStarted then
            F.ensureGuiIndexing()
        end

        if active and guiIndexStarted then
            F.profiledCall("gui-events", function()
                return F.pumpGuiEvents(schedulerEpoch)
            end)
        end
        ...
        elseif not active and modeState.current ~= "Unknown" then
            modeState.current = "Unknown"
        end

        local pentathlonActive =
            CONFIG.enabled.autoPentathlon
            and modeState.current == "Pentathlon"

        -- Exactly one GUI automation owner at a time.
        if pentathlonActive and F.every("pentathlon", 0.070, now) then
            F.profiledCall("pentathlon", F.solvePentathlon)

        elseif CONFIG.enabled.autoQTE
            and not pentathlonActive
            and not sequenceBusy
            and F.every("qte", 0.080, now) then

            F.profiledCall("qte", F.solveGenericQTE)
        end

        if not pentathlonActive and lastPentathlonStage ~= "unknown" then
            F.resetPentathlonInputs()
        end

        if CONFIG.enabled.autoDodge
            and F.every("dodge", CONFIG.performance.dodgeInterval, now) then
            F.profiledCall("dodge", F.autoDodgeTick)
        end

        if CONFIG.enabled.aimAssist
            and F.every("aim", CONFIG.aim.updateInterval, now) then
            F.profiledCall("aim", F.aimTick)
        end

        if CONFIG.enabled.rebelAim
            and modeState.current == "Rebel"
            and F.every("rebel", CONFIG.aim.rebelUpdateInterval, now) then
            F.profiledCall("rebel", F.rebelAimTick)
        end

        if CONFIG.enabled.hideSeekESP
            and modeState.current == "HideAndSeek"
            and F.every("esp", CONFIG.performance.espInterval, now) then
            F.profiledCall("hns-esp", function()
                F.espTick(true)
            end)

        elseif CONFIG.enabled.esp
            and F.every("esp", CONFIG.performance.espInterval, now) then
            F.profiledCall("esp", function()
                F.espTick(false)
            end)

        elseif next(espObjects) ~= nil
            and (not CONFIG.enabled.esp)
            and (not CONFIG.enabled.hideSeekESP or modeState.current ~= "HideAndSeek") then
            F.clearESP()
        end

        if F.every("status", CONFIG.performance.statusInterval, now) then
            local stage = pentathlonActive and F.pentathlonStage() or "-"
            local indexState =
                guiIndexBuilding and "indexing"
                or (guiIndexStarted and "ready" or "idle")

            status.Text = ("Mode:%s/%s | %s | H:%d W:%d | Q:%d/%d/%d/%d | T:%d %.2fms")
                :format(
                    modeState.current,
                    stage,
                    indexState,
                    F.guiHotCount(),
                    guiWatchCount,
                    F.queueCount(indexHead, indexTail),
                    F.queueCount(newGuiHead, newGuiTail),
                    F.queueCount(mutationHead, mutationTail)
                        + F.queueCount(mutationOverflowHead, mutationOverflowTail),
                    F.queueCount(priorityHead, priorityTail),
                    guiStats.processedThisTick,
                    perf.emaMs
                )

            perf.peakMs = perf.peakMs * 0.96
            perf.wallPeakMs = perf.wallPeakMs * 0.98
        end

        task.wait(CONFIG.performance.schedulerInterval)
    end
end)

print("[Ink v8.6.1] Loaded with all automations OFF. GUI indexing is idle until a dependent feature is enabled.")
print("[Ink v8.6.1] Runtime stats: _G.__InkUltimateRuntime.getStats()")
