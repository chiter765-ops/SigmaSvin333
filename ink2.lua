--[[
Ink Game v8.7.0 — cancellable input repair, 2026-09-08.
...
if not LocalPlayer then
    ...
    warn("[Ink v8.7.0] LocalPlayer is unavailable; bootstrap aborted safely.")
    return
end
...
if not PlayerGui then
    ...
    if rawget(_G, RUNTIME_KEY) == runtime then _G[RUNTIME_KEY] = nil end
    warn("[Ink v8.7.0] PlayerGui was unavailable after 10 seconds; bootstrap aborted safely.")
    return
end
...
if game.PlaceId ~= TARGET_PLACE_ID then
    warn(("[Ink v8.7.0] Expected place %d, running in %d. Heuristics may not match.")
...
-- Input driver
-- --------------------------------------------------------------------------
-- Input driver: one backend, tracked holds, no task.wait in taps/clicks.
local VIM = nil
...
function runtime.cancelInput(reason)
    -- Scalar-only; safe to call from a button or input signal. No input APIs,
    -- coroutine joins, queue walks, or cleanup callbacks run on this stack.
...
local function sendInput(method, ...)
    ...
    if not vim then
        runtime.lastError = "VirtualInputManager unavailable"
        runtime.inputBlockedUntil = os.clock() + 3
        return false
    end
    local ok, result = pcall(function(service, ...)
        return service[method](service, ...)
    end, vim, ...)
    if not ok then
        runtime.inputFailures = runtime.inputFailures + 1
        runtime.lastError = "Input: " .. tostring(result)
        runtime.inputBlockedUntil = os.clock() + math.min(5, runtime.inputFailures * 0.5)
    end
    return ok
end
local function keyUp(code)
    if not runtime.heldKeys[code] then return true end
    runtime.heldKeys[code] = nil
    -- Exactly one best-effort release, including after cancellation. Retrying an
    -- inaccessible native API in a tight loop cannot repair the input backend.
...
function runtime.flushInput(force)
    ...
    if runtime.virtualMouseHeld and (force or runtime.mouseEpoch ~= runtime.inputEpoch
        or (runtime.mouseReleaseAt and now >= runtime.mouseReleaseAt)) then
        releaseVirtualMouse()
    end
end
local function keyDown(code)
    ...
    if not sendInput("SendKeyEvent", true, code, false, game) then
        keyUp(code)
        return false
    end
    return true
...
local function keyTap(code, duration)
    ...
    hold.untilAt = os.clock() + math.clamp(duration or 0.018, 0.012, 0.2)
    return true
end
local function mouseClick(position)
    ...
    runtime.mouseEpoch = runtime.inputEpoch
    runtime.mouseReleaseAt = os.clock() + 0.018
    if not sendInput("SendMouseButtonEvent", p.X, p.Y, 0, true, game, 0) then
        releaseVirtualMouse()
        return false
    end
    return true
...
local function clickGuiButton(button)
    if not button or not button:IsA("GuiButton") or not button.Active
        or not isActuallyVisible(button) then return false end
...
local function dragCircle(center, radius, loops, pointsPerLoop, shouldContinue)
    ...
    if not sendInput("SendMouseMoveEvent", start.X, start.Y, game) then return false end
    if not mouseClick(start) then return false end
    runtime.mouseReleaseAt = nil
    local completed = true
    for i = 1, count do
        if epoch ~= runtime.inputEpoch or not runtime.canInput()
            or (shouldContinue and not shouldContinue()) then completed = false; break end
        ...
        trackedInputWait(0.016)
    end
    -- An old drag must never release a newer generation's mouse.
...
-- v8.7.0: semantic identity is deliberately insensitive to dynamic numeric
...
function runtime.snapshotGuiChildren(obj)
    ...
    if not ok or type(children) ~= "table" then
        if CONFIG.debug then
            warn("[Ink v8.7.0] GetChildren snapshot failed:", children)
        end
        return {}
    end
    ...
    if elapsedMs >= CONFIG.performance.slowWorkerMs then
        ...
        if CONFIG.debug then
            warn(("[Ink v8.7.0] slow GetChildren snapshot: %.2fms (%d children)")
...
function F.processIndexQueue()
    ...
    if indexHead > indexTail then
        ...
        if not guiIndexCompletionReported then
            ...
            print(("[Ink v8.7.0] GUI index complete | indexed=%d watchers=%d " ..
...
function F.refreshMode()
    ...
    if modeState.current ~= "Unknown"
        ...
        modeState.changedAt = now
        if CONFIG.debug then
            print("[Ink v8.7.0] Mode -> Unknown (evidence timeout)")
        end
    end
    return modeState.current
end
...
function F.qteContext(obj)
    ...
    for depth = 1, 5 do
        ...
        if containsAny(name, {"chat", "backpack", "shop", "inventory", "settings"}) then return false end
        if containsAny(name, {"qte", "quicktime", "quick_time", "powerhold", "power hold", "clash",
            ...
            "flyingstone", "spinningtop", "jegi", "honeycomb"}) then return true end
        cursor = cursor.Parent
    end
    -- A short explicit instruction is evidence; bare hotbar letters are not.
    local text = lower(textOf(obj))
    return #text <= 32 and text:match("^press%s+") ~= nil
end
function F.promptTiming(obj)
    -- Supports concentric GUI rings, not circles drawn inside a single image.
    -- Unknown image/canvas geometry requires a captured game-specific adapter.
    local root = obj.Parent
    if not root then return false, 0 end
    local objects = F.visibleObjects()
    local ring, diameter = nil, math.huge
    local marker, target = nil, nil
    local center = centerOf(obj)
    for i = 1, #objects do
        local candidate = objects[i]
        if candidate.Parent == root and candidate ~= obj then
            if containsAny(candidate.Name, {"needle", "marker", "pointer"}) then marker = candidate end
            if containsAny(candidate.Name, {"perfect", "successzone", "targetzone", "greenzone"}) then target = candidate end
        end
        if candidate ~= obj and candidate.Parent == root
            and containsAny(candidate.Name, {"ring", "circle", "shrink", "outer"}) then
            local size = candidate.AbsoluteSize
            if size.X > 0 and math.abs(size.X - size.Y) < size.X * 0.2
                and (centerOf(candidate) - center).Magnitude < 10 and size.X < diameter then
                ring, diameter = candidate, size.X
            end
        end
    end
    if not ring then
        if marker and target then
            local inside = pointInside(centerOf(marker), target, -1)
            local state = F.linearSamples[obj] or {cycle = 0, inside = false}
            if inside and not state.inside then state.cycle = state.cycle + 1 end
            state.inside = inside
            F.linearSamples[obj] = state
            return inside, state.cycle
        end
        return true, 0
    end
    local base = math.max(obj.AbsoluteSize.X, obj.AbsoluteSize.Y)
    if base <= 0 then return false, 0 end
    local now = os.clock()
    local state = F.ringSamples[obj]
    if not state or state.ring ~= ring or state.signature ~= F.signature(obj) then
        state = {ring = ring, size = diameter, time = now, cycle = 0, fired = false, signature = F.signature(obj)}
        F.ringSamples[obj] = state
        return false, state.cycle
    end
    if diameter > state.size + base * 0.15 then
        state.cycle = state.cycle + 1
        state.fired = false
    end
    local dt = now - state.time
    local velocity = dt > 0 and (diameter - state.size) / dt or 0
    local predicted = diameter + math.clamp(velocity * 0.016, -base * 0.15, 0)
    local ready = not state.fired and velocity < 0 and predicted <= base * 1.08 and diameter >= base * 0.82
    state.size, state.time = diameter, now
    -- handled[] commits only after the input backend accepts the key/button.
    return ready, state.cycle
end
...
function F.solveKeyPrompts()
    ...
    for i = 1, #objects do
        ...
        if obj:IsA("TextLabel") or obj:IsA("TextButton") then
            ...
            if key and F.qteContext(obj) then
...
function F.resetDalgona()
    ...
    d.status = "Waiting for visible ordered trace points"
end
...
function F.discoverDalgonaPath()
    local d = F.dalgona
    if os.clock() - d.scanAt < 0.5 then return false end
    d.scanAt = os.clock()
    local groups = {}
    for _, obj in ipairs(F.visibleObjects()) do
        local index = lower(obj.Name):match("^tracepoint[_ ]?(%d+)$")
            or lower(obj.Name):match("^checkpoint[_ ]?(%d+)$")
        if index and F.qteContext(obj) and obj.Parent and obj.Parent:IsA("GuiObject") then
            local group = groups[obj.Parent] or {}
            groups[obj.Parent] = group
            group[#group + 1] = {index = tonumber(index), obj = obj}
        end
    end
    for root, nodes in pairs(groups) do
        if #nodes >= 3 and #nodes <= CONFIG.dalgona.maxPoints then
            ...
            local points = {}
            local size, origin = root.AbsoluteSize, root.AbsolutePosition
            local contiguous = size.X > 0 and size.Y > 0
            ...
            if contiguous and runtime.setDalgonaPath(root, points) then return true end
        end
    end
    d.status = "No exposed contour; game UI capture needed"
    return false
end
...
title.Text = "Ink ULTIMATE v8.7.0 CANCELLABLE"
title.Parent = top
...
function F.profiledCall(name, fn)
    ...
    if computeMs >= CONFIG.performance.slowWorkerMs then
        ...
        if CONFIG.debug then
            warn(("[Ink v8.7.0] slow CPU worker %s: %.2fms (wall %.2fms)")
    ...
    if not ok and CONFIG.debug then
        warn("[Ink v8.7.0] worker error [" .. tostring(name) .. "]:", result)
    end
    return ok, result
end
...
function F.applyPendingReset()
    ...
    if guiIndexStarted then enqueuePriority(PlayerGui) end
end
function runtime.captureGui()
    -- A bounded local diagnostic: no remote requests, writes, or script source.
    local snapshot = {version = "8.7.0", mode = modeState.current, objects = {}, stats = runtime.getStats()}
...
function F.tick()
    ...
    if F.every("status", CONFIG.performance.statusInterval, now) then
        ...
        local detail = runtime.lastError ~= "" and runtime.lastError
            or (CONFIG.enabled.autoDalgona and F.dalgona.status or "F5 hide | F6 unload | F7 stop")
...
task.spawn(function()
    while runtime.alive do
        ...
        if not ok then
            ...
            runtime.flushInput(true)
            warn("[Ink v8.7.0] " .. runtime.lastError)
        end
...
print("[Ink v8.7.0] Loaded with all automations OFF. GUI indexing is idle until a dependent feature is enabled.")
print("[Ink v8.7.0] Runtime stats: _G.__InkUltimateRuntime.getStats()")
